...
___doc___ = """
Adds unicode from the Private Use Area to characters that do not have unicode by default
"""
...
class UnicodeFillerGUI:
    def __init__(self):
        self.w = vanilla.Window((340, 150), title="Unicode filler")
        ...
        self.w.integer = vanilla.EditText(
            (90, 40, 200, 25), "57344", callback=self.on_int
        )
    ...
    def fill(self, sender):
        filler = UnicodeFiller(int(self.w.integer.get()), Glyphs.font)
        try:
            filler.run()
        finally:
            self.w.close()
...
class UnicodeFiller:
    def __init__(self, start_unicode, font):
        ...
        self.selected_glyphs = [
            glyph for glyph in font.glyphs if glyph.selected
        ]
    ...
    def check(self):
        ...
        taken_uni = [
            (
                "{}({}={})".format(
                    glyph.name, int(glyph.unicode, 16), glyph.unicode
                )
            )
